...
if podman image exists image-registry.openshift-image-registry.svc:5000/openshift-kmm/sfc-module; then
    echo "Image sfc-module found in the local registry, removing in-tree kernel module"

    podman run --privileged --entrypoint modprobe image-registry.openshift-image-registry.svc:5000/openshift-kmm/sfc-module -rd /opt sfc sfc_driverlink
    if [ $? -eq 0 ]; then
    ...
    podman run --privileged --entrypoint modprobe image-registry.openshift-image-registry.svc:5000/openshift-kmm/sfc-module -d /opt sfc
    if [ $? -eq 0 ]; then
